...
- **Автоплей + перехват.** Приехавшая механика крутит демо под вуалью
  (`game__autoplay`; обычная лента сохраняет рамку со всех четырёх сторон, а
  exact candidate/dev session равномерно масштабирует всю поверхность без
  искажения aspect ratio); тап — плавный ease-out перехват в ручную игру; свайпы
...
| `?helpmap=1` | Dogfood-превью «карты помощи» (то же — `start_param=helpmap`): вкладка «Мета» становится видимой и ведёт в карту. |
...
За `startapp=helpmap` (или `?helpmap=1`) вкладка «Мета» перестаёт быть скрытой и
открывает карту помощи поверх ленты. Это **превью, а не мета**: данные
демонстрационные, бейдж «прототип · демо-данные» намеренно остаётся на экране,
запросов к API нет. Без параметра нет ни вкладки, ни загрузки карты.

Карта живёт в Shadow DOM, а её геометрия и рантайм лежат **рядом** с бандлом
(`public/helpmap/`, в сборке — `dist/helpmap/`) и подтягиваются только при
открытии: `vite-plugin-singlefile` инлайнит любой импорт, поэтому положить их
внутрь бандла означало бы +258 КБ gzip к `index.html` на каждого пользователя.

Источник правды — прототип в `../help-map-prototype`; клиентские артефакты
генерируются из него и руками не правятся:

```
cd ../help-map-prototype && node build-client.mjs --out=../feed-prototype/public/helpmap
```

Структурный гейт: `npm run check:helpmap-preview`.
...
## Публикация из Dev-ленты

Код публикации копируется по нажатию; подтверждение остаётся отдельным действием.
После отправки панель показывает серверный результат по каждой выбранной механике:
«Ожидает публикации», «Публикуется», «Опубликовано» или «Не выполнено». Если работа
начиналась, но сервер не подтвердил исход, показывается «Статус не подтверждён».
Сбой проверки статуса не означает сбой самой публикации.

В открытой панели статусы проверяются каждые 5 секунд, до 24 проверок; затем
доступна кнопка «Проверить статус». Закрытая или фоновая панель не опрашивает
сервер. Отслеживание хранится только в текущем открытии приложения: после полной
перезагрузки итог проверяется в Release-ленте и Telegram. Проверка статуса никогда
не повторяет публикацию и не запрашивает новый код.

## Research с телефона (#188)

В существующем экране `LAB` кнопка «Новая заявка Research» появляется только
после успешного серверного `GET /api/operator/research-parties/capability`
с `enabled: true`. Выключенная capability или `404` оставляет вход скрытым,
не мешая прежнему device-flow. Research остаётся **default-off**: этот код
не включает Backend/worker, не выдаёт grant и не меняет настройки.

- Создание: TMA `start_param=rp_new` либо `?researchParty=new`.
  Источники автоматической партии — Poki/CrazyGames; Meta/TikTok недоступны
  без присутствия человека. Линза R/D и лимит обращений (1–50, не больше
  серверного остатка) выбираются явно. «Создать заявку» означает сохранение
  заявки, **не готовый Research**.
- Результат: `start_param=rp_<requestId>` либо
  `?researchParty=<requestId>` с каноническим UUID. После создания кнопка
  «Открыть результат» открывает тот же экран без изменения подписанного
  TMA `start_param`; reload восстанавливает сохранённую заявку.
  Показаны ожидание, `NEEDS_HELP` либо `READY` с 5–7 карточками. Старый `READY`
  без shortlist честно остаётся без выбора, а не превращается в пустой успех.
- Создание и выбор/отклонение карточки — отдельные явные действия, без
  выбора по умолчанию, публикации или автоматического запуска игр. Точный
  pending-command записывается в `sessionStorage` текущего аккаунта до POST;
  потерянный ответ восстанавливается чтением Backend, не автоматическим
  повтором POST или новым mutation ID. При неподтверждённом исходе доступна
  явная кнопка повторной отправки **того же сохранённого** решения/заявки:
  actor, command, mutation и hash не меняются; новые действия остаются
  заблокированы до подтверждения либо явного отказа сервера. При недоступном
  или повреждённом storage отправки нет. Неопределённый исход не выдаётся за отказ или успех.

Оба экрана используют текущую TMA-авторизацию через `getRequired/postRequired`:
без worker bearer в браузере и без localhost proxy. Device grant
`research.party:write` привязан только к Mac B; это не право выбирать за человека.
Research-entry проверяется **до** `labAuth`: сочетание `researchParty` с
`labAuth` или другой конфликтующей целью открывает сообщение о недействительной
Research-ссылке, без fallback в device approval или обычную ленту. Конфликт
подписанного `start_param` с Research-целью также не разрешается автоматически.

Три состояния учитываются отдельно: **реализовано** — код и синтетические
wire/browser-проверки этого пакета; **доступно в клиенте** — только после
проверенной доставки совместимых Backend/Feed и разрешённого enablement;
**проверено человеком** — отдельный реальный phone-proof, здесь не заявлен.
Актуальные delivery/acceptance receipts — в [meta #188](https://github.com/Glera/p4g-workspace-meta/issues/188).
Обязательный полный PR CI включает `check:research-party`,
`check:research-party-browser`, `check:research-party-create-browser` и прежние
Lab-проверки. [Pinned fixtures](scripts/fixtures/research-party/README.md)
не заменяют live-проверку источников или телефонную приёмку.
