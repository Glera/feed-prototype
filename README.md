...
- 🐞 — дебаг-панель; видна только в dev-сборке, при `?diag=1` или
  `start_param=diag`.
...
- **Один device tier на страницу.** Parent-платформа до монтирования первого
  игрового iframe один раз измеряет Canvas2D в видимом Telegram-контексте,
  кэширует versioned результат и передаёт одинаковый `quality`/`tier` всем
  built-in, story/island и catalog iframe. Скрытые warm-механики не запускают
  собственный benchmark и не могут ложно уронить всю ленту в `low`.
...
Дебаг-панель открывается `?diag=1`, `t.me/<bot>?startapp=diag` или кнопкой 🐞 в
dev-сборке: initData/auth, статус `/session`, живой лог событий, сброс своего
...
| `?deviceTier=premium\|standard\|low` | Dev/QA override parent-owned device tier; тот же tier получают все iframe страницы. |
