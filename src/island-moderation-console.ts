...
} from './island-moderation-pagination.mjs';
...
export async function mountIslandModerationConsole(): Promise<void> {
  ...
  const requestGeneration = createModerationRequestGeneration();
  const buildingMutations = new Set<string>();
  const reportMutations = new Set<string>();
  const paginationMutations = new Set<string>();
  ...
  async function reload(): Promise<void> {
    ...
    try {
      const [pubs, reports] = await Promise.all([
        apiIslandModerationPublications(),
        apiIslandModerationReports({ status: filter }),
      ]);
  ...
  async function loadMorePubs(): Promise<void> {
    ...
    const before = pubNextBefore;
    try {
      const r = await apiIslandModerationPublications({ before });
      if (!requestGeneration.isCurrent(generation)) return;
      pubItems = pubItems.concat(r.publications);
      pubNextBefore = r.next_before;
      paint();
    } catch (error) {
      if (requestGeneration.isCurrent(generation)) toast(`ещё: ${errText(error)}`);
    }
  }
  ...
  async function loadMoreReports(): Promise<void> {
    ...
    const generation = requestGeneration.capture();
    const before = reportNextBefore;
    const filter = reportsFilter;
    try {
      const r = await apiIslandModerationReports({ status: filter, before });
      if (!requestGeneration.isCurrent(generation)) return;
  ...
  const moreBtn = (
    ...
    const b = mkBtn(label, async () => {
      await runKeyedModerationAction(paginationMutations, key, [b], fn);
    });
  ...
  function renderPublications(pubs: IslandModerationPublication[]): HTMLElement {
    ...
    for (const p of pubs) {
      ...
      if (p.taken_down) {
        row.appendChild(mkBtn('Restore', (b) => armThen(b, 'ещё раз = вернуть', async () => {
          await runKeyedModerationAction(buildingMutations, p.building_id, [b], async () => {
        ...
        row.appendChild(mkBtn('Takedown', (b) => armThen(b, 'ещё раз = снять', async () => {
          ...
          const artifactRel = p.rel;
          await runKeyedModerationAction(buildingMutations, p.building_id, [b], async () => {
            try { await apiIslandModerationTakedown(p.building_id, artifactRel, 'operator takedown'); toast('снято'); await reload(); }
    ...
    if (pubNextBefore) {
      section.appendChild(moreBtn('publications', '▾ Показать ещё', loadMorePubs));
    }
    return section;
  }
  ...
  function renderReports(reports: IslandModerationReport[]): HTMLElement {
    ...
    for (const r of reports) {
      ...
      const row = document.createElement('div');
      row.style.cssText = 'display:flex;gap:8px;flex-wrap:wrap;';
      const resolveButtons: HTMLButtonElement[] = [];
      for (const status of ['reviewed', 'dismissed', 'escalated'] as const) {
        const resolveButton = mkBtn(status, async () => {
          await runKeyedModerationAction(reportMutations, r.report_id, resolveButtons, async () => {
        ...
        resolveButtons.push(resolveButton);
    ...
    if (reportNextBefore) {
      section.appendChild(moreBtn('reports', '▾ Показать ещё', loadMoreReports));
    }
    return section;
  }
