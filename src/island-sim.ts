...
// plays/likes dealt by a simulation, plus `reward` — uncollected puzzles that pile
// up over a building until the owner taps to collect them.
//
...
// on the feed tab, and puzzles must keep accruing over mechanics regardless of
// which view is open. The island overlay reads the same state to render + collect.

export interface IslandSim {
  plays: Record<number, number>;
  likes: Record<number, number>;
  reward: Record<number, number>;
}
...
// refresh (updated plays/likes + newly-arrived puzzle pucks) without owning the loop.
